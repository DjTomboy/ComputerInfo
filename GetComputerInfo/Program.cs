...
{
    internal static StringBuilder errorCollector = new();
    private static void Main()
    {
        //AppUpdate után megmaradó bak fájlok törlése
        AppUpdate.DeleteBakFiles();
        
        //Frissítés ellenőrzése
        Console.WriteLine("Program frissítés ellenőrzése...");
        if (Operations.CheckForUpdate())
        {
            Console.Write("Az alkalmazásból elérhető újabb verzió. Frissítés indítása? (y/N) ");
            if (Console.ReadKey().Key == ConsoleKey.Y)
            {
                Console.WriteLine();
                Console.WriteLine("Frissítés indítása...");
